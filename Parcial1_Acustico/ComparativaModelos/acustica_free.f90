...
!===========================================================
!===========================================================
!         PROPAGACION ACUSTICA EN EL OCEANO
!===========================================================
!===========================================================
...
!                             ! CONDICIONES INICIALES
        ...
        CALL DERIVS(Y,DYDX)
!                             ! CICLO SOBRE EL INTERVALO S
        DO L=1,5*INT(PRR)
          S=S+HS
          CALL RK4(Y,DYDX,N,HS,YOUT)
          DO I=1,N
!                             ! CONDICION REBOTE EN LA
!                             ! SUPERFICIE O FONDO
  ...
  100   WRITE (97,'()')
      END DO
!===========================================================
!     SOLUCION DE LA ECUACION DE HELMHOLTZ MEDIANTE
!     EL METODO DE MODOS NORMALES
!     PASO
      HZ=D/FLOAT(NPT)
!                             ! RANGOS DE BUSQUEDA
!                             ! DE LOS MODOS
      ...
      END DO
!===========================================================
!     CALCULO DE LAS FUNCIONES NORMALIZADAS
      DO I=1,NPT
        C(I)=VSON(FLOAT(I)*HZ)
      END DO
      DO J=1,JJ
...
!                             ! NORMALIZADA
      ...
      END DO
!===========================================================
!     DETERMINACION DE LA PERDIDA POR TRANSMISION (EC 20)
      DO IP=1,INT(D/100)
        DO I=1,INT(PRR)
          R=FLOAT(I)
          SUM=0.D0
...
!     ECUACIONES DIFERENCIALES PARA EL TRAZADO
!     DE RAYOS (EC 7,8)
      CS=VSON(Y(2))
      DYDX(1)=CS*Y(3)
      DYDX(2)=CS*Y(4)
...
!===========================================================
      FUNCTION VSON(Z)
      IMPLICIT REAL*8(A-H,O-Z)
!     PERFIL DE VELOCIDAD DE MUNK (EC 33)
...
!===========================================================
      FUNCTION VDER(Z)
      IMPLICIT REAL*8(A-H,O-Z)
!     DERIVADA DEL PERFIL DE VELOCIDAD DE MUNK
      PARAMETER (V0=1500.D0, EPS=0.00737D0, ZM=1300.D0)
!     DERIVADA DE LA VELOCIDAD DEL SONIDO
      G=2.D0*(Z-ZM)/ZM
...
!===========================================================
      FUNCTION DETERM(X)
      IMPLICIT REAL*8(A-H,O-Z)
!     CALCULO DEL DETERMINANTE (EC 30)
      DIMENSION P(0:2000), C(2000)
      COMMON /DET/ W, H, N
!     CONSTRUCCION DEL VECTOR VELOCIDAD
      DO I=1,N
        C(I)=VSON(H*FLOAT(I))
      END DO
      P(0)=0.D0
...
!     NORMALIZACION DEL VECTOR
      ...
      END
!===========================================================
!     SUBRUTINA RK4 - INTEGRADOR RUNGE-KUTTA ORDEN 4
!     Numerical Recipes in Fortran, Cap. 16
!     Integra un sistema de N ODEs un paso H
!     Y(N)    : valores actuales
!     DYDX(N) : derivadas en el punto actual
!     N       : numero de ecuaciones
!     H       : paso de integracion
!     YOUT(N) : solucion al paso siguiente
!===========================================================
...
!     TERCER PASO
      ...
      END DO
      CALL DERIVS(YT,DYT)
!     COMBINACION DE LOS CUATRO PASOS
      DO I=1,N
      ...
      END
!===========================================================
!     SUBRUTINA ZBRAK - BUSQUEDA DE BRACKETS DE RAICES
!     Numerical Recipes in Fortran, Cap. 9
!     Divide el intervalo [X1,X2] en N subintervalos y
!     detecta cambios de signo de FUNC (posibles raices)
!     XB1, XB2 : extremos de los brackets encontrados
!     NB (entrada): maximo de brackets buscados
!     NB (salida) : numero de brackets encontrados
!===========================================================
      ...
      END
!===========================================================
!     FUNCION ZBRENT - REFINAMIENTO DE RAICES (METODO DE BRENT)
!     Numerical Recipes in Fortran, Cap. 9
!     Encuentra la raiz de FUNC en [X1,X2] con tolerancia TOL
!     Combina biseccion, secante e interpolacion inversa
!     Garantiza convergencia si hay cambio de signo en [X1,X2]
!===========================================================
...
!           INTERPOLACION LINEAL (SECANTE)
            ...
            Q=1.D0-S
          ELSE
!           INTERPOLACION INVERSA CUADRATICA
            Q=FA/FC
            R=FB/FC
...
!         BISECCION
      ...
      END
!===========================================================
!     SUBRUTINA TRIDAG - SOLVER TRIDIAGONAL (ALGORITMO DE THOMAS)
!     Numerical Recipes in Fortran, Cap. 2
!     Resuelve el sistema tridiagonal: A*u = r
!     A(J) : subdiagonal  (j=2..N)
!     D(J) : diagonal principal (j=1..N)
!     C(J) : superdiagonal (j=1..N-1)
!     R(J) : vector del lado derecho
!     U(J) : solucion
!===========================================================
      ...
      BET=D(1)
      U(1)=R(1)/BET
!     DESCOMPOSICION Y SUSTITUCION HACIA ADELANTE
      DO J=2,N
        GAM(J)=C(J-1)/BET
        BET=D(J)-A(J)*GAM(J)
        IF(BET.EQ.0.D0) THEN
